...
from drone_interfaces.srv import ToggleVelocityControl, SetServo
...
class FollowDetections(DroneController):
    def __init__(self):
        ...
        # --- Gimbal: SetServo (universalny: symulacja + prawdziwy dron) ---
        self.servo_client = self.create_client(SetServo, '/knr_hardware/set_servo')
        while not self.servo_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('SetServo service unavailable, waiting...')
        ...
        self._send_servo_pwm(self._angle_to_pwm(self.gimbal_angle_deg))
    ...
    # ── Gimbal: konwersja kąt → PWM ──────────────────────────
    def _angle_to_pwm(self, angle_deg: float) -> int:
        """Konwertuj kąt gimbala (stopnie) na PWM dla SetServo.
        
        Mapowanie (z SDF CameraJoint 0..1.05 rad + bazowy pitch 30°):
          30° (prosto-do przodu) → PWM 500  (ActuatorServos=0.0)
          60° (pochylone)        → PWM 760  (ActuatorServos=0.52)
          90° (prosto w dół)     → PWM 1000 (ActuatorServos=1.0)
        """
        angle_deg = clamp(angle_deg, 30.0, 90.0)
        pwm = 500 + int(((angle_deg - 30.0) / 60.0) * 500)
        return clamp(pwm, 500, 1000)

    def _send_servo_pwm(self, pwm: int):
        """Wyślij komendę PWM do serwa kamery (fire-and-forget)."""
        req = SetServo.Request()
        req.servo_id = 8  # CameraJoint → servo_7 (index = servo_id - 1)
        req.pwm = int(pwm)
        if self.servo_client.service_is_ready():
            self.servo_client.call_async(req)

    def set_gimbal_angle(self, angle_degrees):
        """Ustaw kąt gimbala w stopniach (30°..90°). Blokujący."""
        self.get_logger().info(f"Gimbal → {angle_degrees}°")
        self.gimbal_angle_deg = float(angle_degrees)
        pwm = self._angle_to_pwm(angle_degrees)
        self._send_servo_pwm(pwm)
