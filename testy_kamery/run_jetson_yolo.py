...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="YOLO stream from OAK PoE with styled boxes")
    parser.add_argument("--ip", default="169.254.1.222", help="OAK device IP")
    parser.add_argument(
        "--model",
...
def setup_cuda() -> bool:
    ...
    # Jetson Orin fix: disable benchmark mode to avoid missing engine errors
...
def connect_with_retry(
    pipeline: dai.Pipeline, ip: str, retries: int = 20, delay_sec: float = 1.0
) -> dai.Device:
    last_exc = None
    for attempt in range(1, retries + 1):
        try:
            ...
            last_exc = exc
            if attempt < retries:
                print(f"Waiting for OAK device ({attempt}/{retries})...")
...
def main() -> None:
    ...
    # Warm up CUDA before entering the camera loop
    ...
    with connect_with_retry(pipeline, args.ip) as device:
        video_queue = device.getOutputQueue("rgb", maxSize=4, blocking=False)
        print(f"YOLO stream from {args.ip} started on [{current_device}]. Press q to quit.")
        ...
        try:
            while True:
                ...
                cv2.imshow("YOLO OAK PoE", annotated)
