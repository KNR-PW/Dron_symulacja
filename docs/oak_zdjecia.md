...
```bash
ros2 launch drone_bringup oak_photos.launch.py camera_topic:=/tent_detections/image fps:=1.0
```
...
ros2 launch drone_bringup oak_photos.launch.py second:=true \
  camera_topic:=/oak/rgb/image_raw      fps:=1.0 \
  camera_topic2:=/tent_detections/image  fps2:=1.0
```
